...
function Bill({ type }) {
	...
	let options = (type) => {
		return type === 'Income' ? ['salary', 'gift', 'business'] : ['recharge', 'groceries', 'fun'];
	};
	const currentDate = {
	...
	let defaultValue = (type) => {
		return type === 'Income' ? 'salary' : 'recharge';
	};

	const initVals = { name: defaultValue(), price: '', date: new Date().toLocaleDateString() }
